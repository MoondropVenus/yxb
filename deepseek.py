...
async def take_screenshot(tab, filename="deepseek_exam.png"):
    """截图考试页面"""
    print("4. 正在截图考试页面...")
    try:
        # 截图整个页面
...
def call_deepseek_api(question_text):
    """调用DeepSeek API解答问题"""
    print("6. 正在使用DeepSeek解答问题...")
    try:
        # 配置DeepSeek API
        client = OpenAI(
            base_url='https://api.deepseek.com',
            api_key='sk-7c672611a0414600b47dd79f40090a9b',  # DeepSeek API密钥
        )
       ...
       题目：
        ...
        print(f"   DeepSeek解答失败: {e}")
        # 不提供默认答案，返回None表示解答失败
        print(f"\nDeepSeek解答失败，不提供默认答案")
        return None, ""
...
def save_results(screenshot_path, extracted_text, model_answer):
    ...
    try:
        # 保存题目内容
        with open("deepseek_exam_content.txt", "w", encoding="utf-8") as f:
            f.write("考试题目内容:\n")
            f.write("=" * 50 + "\n")
            f.write(extracted_text if extracted_text else "未能提取到题目内容")
        
        # 保存AI答案
        with open("deepseek_exam_answers.txt", "w", encoding="utf-8") as f:
            f.write("考试题目:\n")
            f.write("=" * 50 + "\n")
            f.write(extracted_text if extracted_text else "未能提取到题目内容")
            f.write("\n\nDeepSeek答案:\n")
            f.write("=" * 50 + "\n")
            f.write(model_answer if model_answer else "未能获得DeepSeek答案")
        
        print("   结果已保存到:")
        print("   - deepseek_exam_content.txt (题目内容)")
        print("   - deepseek_exam_answers.txt (题目和答案)")
        if screenshot_path:
...
async def auto_solve_exam(browser, tab):
    ...
    while question_count < total_questions:
        ...
        print("正在调用DeepSeek解答问题...")
        ai_answer, ai_explanation = call_deepseek_api(question_text)
        if ai_answer:
            print(f"DeepSeek答案: {ai_answer}")
        else:
            print("DeepSeek解答失败")
...
async def main():
    ...
    try:
        ...
        # 7. 保存最终结果
        if extracted_text:
            save_results(screenshot_path, extracted_text, "DeepSeek答案已显示在终端")
